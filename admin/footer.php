  ...
  <script>
    ...
    datasets: [{
        ...
        data: [
            ...
            for ($bulan = 1; $bulan <= 12; $bulan++) {
                $query = "SELECT SUM(transaksi_nominal) AS total_pemasukan 
                          FROM transaksi 
                          WHERE transaksi_jenis='Pemasukan' 
                          AND MONTH(transaksi_tanggal) = ? 
                          AND YEAR(transaksi_tanggal) = ?";
                
                $stmt = mysqli_prepare($koneksi, $query);
                mysqli_stmt_bind_param($stmt, "ii", $bulan, $thn_ini);
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);
                $pem = mysqli_fetch_assoc($result);

                $total = isset($pem['total_pemasukan']) ? $pem['total_pemasukan'] : 0;
                echo $total . ",";
            }
            ?>
        ]
    },
    {
        ...
        data: [
            ...
            $thn_ini = 2023;
            for ($bulan = 1; $bulan <= 12; $bulan++) {
                $query = "SELECT SUM(transaksi_nominal) AS total_pengeluaran 
                          FROM transaksi 
                ...
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);
                $peng = mysqli_fetch_assoc($result);

                $total = isset($peng['total_pengeluaran']) ? $peng['total_pengeluaran'] : 0;
                echo $total . ",";
            }
            ?>
        ]
